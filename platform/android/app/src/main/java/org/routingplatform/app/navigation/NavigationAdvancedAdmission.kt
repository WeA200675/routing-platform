...
object OfflineRoutingProvenanceAdmission {
    fun verified(provenance: OfflineRoutingProvenance?): Boolean {
        if (provenance == null || provenance.sourceId.isBlank() || provenance.version.isBlank()) return false
...
object NavigationStateRestoreAdmission {
    ...
    fun mayRestore(
        ...
        if (currentBootId.isBlank() || state.bootId.isBlank() || state.bootId != currentBootId) return false
